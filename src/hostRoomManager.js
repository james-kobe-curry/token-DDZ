...
export class HostRoomManager {
  ...
  createRoom({ name }) {
    ...
    const player = { id: `p-${createToken(8)}`, token, seat: 0, name: cleanName(name), ready: false, rematchReady: false, connected: true, lastSeq: 0, isBot: false };
  ...
  joinRoom({ code, name }) {
    ...
    const token = createToken();
    const player = { id: `p-${createToken(8)}`, token, seat, name: cleanName(name), ready: false, rematchReady: false, connected: true, lastSeq: 0, isBot: false };
  ...
  async setReady(code, token, ready) {
    ...
    room.updatedAt = this.now();
    if (roomCanStart(room)) room.game = await createRound(this.now());
  ...
  async setBots(code, token, count) {
    ...
    while (bots.length < count) {
      ...
      const bot = { id: `bot-${createToken(8)}`, token: null, seat, name: BOT_NAMES[seat - 1], ready: true, rematchReady: true, connected: true, lastSeq: 0, isBot: true, difficulty: 'master' };
